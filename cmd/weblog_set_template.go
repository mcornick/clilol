...
var (
	weblogSetTemplateFilename string
	weblogSetTemplateCmd      = &cobra.Command{
		Use:   "template",
...
from stdin.`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			var result Result
			var content string
			if weblogSetTemplateFilename != "" {
				input, err := os.ReadFile(weblogSetTemplateFilename)
				cobra.CheckErr(err)
...
func init() {
	weblogSetTemplateCmd.Flags().StringVarP(
		&weblogSetTemplateFilename,
		"filename",
		"f",
		"",
		"file to read template from (default stdin)",
	)
	weblogSetCmd.AddCommand(weblogSetTemplateCmd)
}
