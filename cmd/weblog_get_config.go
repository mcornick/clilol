...
var (
	weblogGetConfigFilename string
	weblogGetConfigCmd      = &cobra.Command{
		Use:   "config",
			...
			if !silent {
				if !wantJson {
					if result.Request.Success {
						if weblogGetConfigFilename != "" {
							err = os.WriteFile(
								weblogGetConfigFilename,
...
func init() {
	weblogGetConfigCmd.Flags().StringVarP(
		&weblogGetConfigFilename,
		"filename",
		"f",
		"",
	...
	weblogGetCmd.AddCommand(weblogGetConfigCmd)
}
