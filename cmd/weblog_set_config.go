...
var (
	weblogSetConfigFilename string
	weblogSetConfigCmd      = &cobra.Command{
		Use:   "config",
...
from stdin.`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			var result Result
			var content string
			if weblogSetConfigFilename != "" {
				input, err := os.ReadFile(weblogSetConfigFilename)
				cobra.CheckErr(err)
...
func init() {
	weblogSetConfigCmd.Flags().StringVarP(
		&weblogSetConfigFilename,
		"filename",
		"f",
		"",
		"file to read config from (default stdin)",
	)
	weblogSetCmd.AddCommand(weblogSetConfigCmd)
}
